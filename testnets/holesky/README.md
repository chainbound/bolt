...
building is not supported yet. You can download Geth from [the official
...
MEV-Boost fork that supports the [Constraints API](https://docs.boltprotocol.xyz/api/builder).

Both services require a configuration file with environment variables. For the
Bolt sidecar you need to create a `bolt-sidecar.env` file in this folder; if you
need a reference template you can checkout the
[`.env.example`](../../bolt-sidecar/.env.example). To
configure properly the signing options it is strongly recommended to read the
[delegations and
signing](#delegations-and-signing-options-for-standalone-and-docker-container-setup)
reference section of this guide.

For the MEV-Boost fork you need to create a `mev-boost.env` file in this folder.

In order to launch the Bolt sidecar as a Docker container you need to create a
`bolt-sidecar.env` configuration file in this folder. For a reference template you can
checkout the [`.env.example`](../../bolt-sidecar/.env.example) file.
If you want to [avoid restarting the beacon
node](#avoid-restarting-the-beacon-node) please follow the steps in the linked
section.

After you've set up the configuration file you can build the Docker container by
running:

```bash
cd testnets/holesky && docker compose build
```
...
and RPCs. This step is needed to prove that your commitments are economically
backed.
...
Then you need to interact with two contracts on Ethereum Mainnet:
...
If you're using the Docker container setup make sure you have [Rust](https://www.rust-lang.org/tools/install) installed in your system.
Then you can build the `bolt-delegations-cli` binary by running:
